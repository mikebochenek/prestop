...
case class UserSettings(id: Long, var language: String, skipDailyEmail: Boolean, 
    ...
    var deviceOS: Option[String], var deviceSWidth: Option[Double], var deviceLang: Option[String],
    var currentPaymentPlan: Option[String])
    
object UserSettings {
  def default(id: Long) = { new UserSettings(id, "en_US", false, "", 0, MutableList.empty[Cuisine], 
      Option(MutableList.empty[Cuisine]), Option(MutableList.empty[Cuisine]), 
      Option(""), Option(0.0), Option(""), Option("")) }
