  ...
  def findDishLikers(dish_id: List[Long], user_id: Long): Seq[DishLikers] = {
    DB.withConnection { implicit connection =>
      SQL("""
        ...
        inner join activity_log a on a.user_id = f.friend_user_id and a.activity_type = 11 and a.activity_subtype in ({dish_id})
        where f.user_id = {user_id}
      """).on('user_id-> user_id, 'dish_id -> (dish_id.mkString(","))).as(Friend.simpleDishLikers *)
    }
  }
